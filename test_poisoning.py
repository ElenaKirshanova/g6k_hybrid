    ...
    for nrand_param in nrand_params:
        ...
        for approx_fact in approx_facts:
            ...
            for tstnum in range(ntests):
                print(f" - - - {approx_fact} #{tstnum} out of {ntests} - - - nrand: {nrand_param} --- lat# {cntr}", flush=True)
                ...
                if not succ_bab:
                    ...
                    try:
                        ...
                        if poison_dbt and not succ: 
                            #since we failed to recover the correct target, we are now
                            #checking that the recovered error is correct for some poisoned target
                            out_reduced = to_canonical_scaled( G,out_gs_reduced,offset=sieve_dim,scale_fact=gh )
                            mindist_to_lattice = float("inf")
                        
                            for vpoison in poison:
                                t_gs_poisoned = t_gs + vpoison
                                t_poisoned = to_canonical_scaled( G,t_gs_poisoned,offset=sieve_dim,scale_fact=gh )
                                vcand = t_poisoned - out_reduced
                                mindist_to_lattice = min( mindist_to_lattice, dist_babai(g6k.M,vcand) )
                                if mindist_to_lattice < 0.1:
                                    break
                            print(f"mindist_to_lattice: {mindist_to_lattice} ")
                            if mindist_to_lattice > 1.44:
                                print(f"Somethings wrong")
                            # assert (mindist_to_lattice < 1.44), "Slicer works incorrectly!"
